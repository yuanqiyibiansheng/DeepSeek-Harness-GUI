...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
    ...
    list(id: SessionId): RpcResult<readonly CommandDescriptor[]> {
      ...
      return {
        ...
        value: [
          ...
          { name: 'goal', description: '设置或查看长期任务的目标', input: { hint: '<objective>' } },
          { name: 'permission', description: '切换权限预设（沙箱模式 + 审批策略）', input: { hint: '<preset>' } },
          { name: 'plan', description: '进入或退出规划模式', input: { hint: '[off|message]' } },
        ],
      }
    },
  ...
  const api: ApiProxy = {
    ...
    host: {
      ...
      readInstructions: request => ok(request, { content: null }),
      writeInstructions: request => ok(request, { written: true as const }),
    },
    workspace: {
    ...
    skills: {
      ...
      listManaged: (request) => ok(request, {
        skills: [
          { name: 'fixture-demo', description: 'fixture 技能样本', modelInvocable: true, userInvocable: true, path: '/t/.dsh/skills/fixture-demo/SKILL.md', loadable: true },
          { name: 'fixture-user-only', description: 'fixture 仅用户技能样本', modelInvocable: false, userInvocable: true, path: '/t/.dsh/skills/fixture-user-only/SKILL.md', loadable: true },
        ],
      }),
      updateManaged: (request) => ok(request, {
        name: request.payload.name,
        description: 'fixture 技能样本',
        modelInvocable: request.payload.toggle.modelInvocable ?? true,
        userInvocable: request.payload.toggle.userInvocable ?? true,
        path: `/t/.dsh/skills/${request.payload.name}/SKILL.md`,
        loadable: true,
      }),
      removeManaged: (request) => ok(request, { removed: true }),
    },
    goals: {
    ...
    pluginMarket: {
      snapshot: request => ok(request, { market: [], installed: [] }),
      install: request => ok(request, { installed: true as const }),
      uninstall: request => ok(request, { removed: true as const }),
      update: request => ok(request, { updated: true as const }),
    },
  }
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'host.readInstructions': return this.api.host.readInstructions(request, new AbortController().signal)
      case 'host.writeInstructions': return this.api.host.writeInstructions(request, new AbortController().signal)
      ...
      case 'skill.updateManaged': return this.api.skills.updateManaged(request)
      case 'skill.removeManaged': return this.api.skills.removeManaged(request)
      case 'pluginMarket.snapshot': return this.api.pluginMarket.snapshot(request)
      case 'pluginMarket.install': return this.api.pluginMarket.install(request)
      case 'pluginMarket.uninstall': return this.api.pluginMarket.uninstall(request)
      case 'pluginMarket.update': return this.api.pluginMarket.update(request)

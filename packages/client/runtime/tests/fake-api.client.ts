...
export class FakeApiClient implements IApiClient {
  ...
  readonly host: IApiClient['host'] = {
    ...
    readInstructions: (payload: unknown) => this.record('host.readInstructions', payload, Promise.resolve(ok({ content: null }))),
    writeInstructions: (payload: unknown) => this.record('host.writeInstructions', payload, Promise.resolve(ok({ written: true }))),
  }
  ...
  readonly skills: IApiClient['skills'] = {
    ...
    listManaged: (payload: unknown) => this.record('skill.listManaged', payload, Promise.resolve(ok({ skills: [] }))),
    updateManaged: (payload: unknown) => this.record('skill.updateManaged', payload, Promise.resolve(ok({ name: (payload as { name: string }).name, description: '', modelInvocable: true, userInvocable: true, path: '', loadable: true }))),
    removeManaged: (payload: unknown) => this.record('skill.removeManaged', payload, Promise.resolve(ok({ removed: true }))),
  }

  readonly pluginMarket: IApiClient['pluginMarket'] = {
    snapshot: (payload: unknown) => this.record('pluginMarket.snapshot', payload, Promise.resolve(ok({ market: [], installed: [] }))),
    install: (payload: unknown) => this.record('pluginMarket.install', payload, Promise.resolve(ok({ installed: true }))),
    uninstall: (payload: unknown) => this.record('pluginMarket.uninstall', payload, Promise.resolve(ok({ removed: true }))),
    update: (payload: unknown) => this.record('pluginMarket.update', payload, Promise.resolve(ok({ updated: true }))),
  }
  ...
  readonly vision: IApiClient['vision'] = {
    status: payload => this.record('vision.status', payload, Promise.resolve(ok({ enabled: false, configured: false, model: 'qwen3.8-max', apiKeyUrl: 'https://help.aliyun.com/zh/model-studio/get-api-key' }))),
    test: payload => this.record('vision.test', payload, Promise.resolve(ok({ model: 'qwen3.8-max', description: 'fake' }))),
    enable: payload => this.record('vision.enable', payload, Promise.resolve(ok({ model: 'qwen3.8-max', description: 'fake' }))),
  }

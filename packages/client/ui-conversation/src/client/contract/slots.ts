...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * Action strip attached to one user-sent message, rendered inside that
     * message's IconActions row. The chat entry owns the render site and
     * passes the durable node sequence as the addressed identity.
     */
 ...
 */
/** Owner currency of the user-message action strip: the durable node seq. */
export interface UserActionOwnerProps {
  /** Stable identity carried from the user message node. */
  messageId: string
}

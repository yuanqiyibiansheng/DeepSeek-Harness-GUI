...
/** Contribute the lazy inventory tab and the plugin market tab to the Plugins settings section. */
export function apply(ctx: ClientContext): void {
  ...
  // The plugin market tab owns the pluginMarket wire face.
  const connection = ctx.get('connection') as ConnectionHandle
  const marketInjected = (): PluginMarketTabInjected => ({ api: connection.api })

  ctx.slots.inject('settings.plugins.tab', () => ctx.slots.register({
    name: 'settings.plugins.tab',
    id: 'market',
    order: 0,
    label: () => t('marketSearch'),
    locale: NS,

...
export function CustomProviderCard(props: CustomProviderCardProps): ReactNode {
  ...
  /** Apply a one-click preset, filling the form so the user can adjust and save. */
  const applyPreset = (preset: ProviderPreset): void => {
    setRoute(preset.route)
    setDisplayName(preset.displayName)
    setBaseURL(preset.baseURL ?? '')
    // Only protocols the adapter actually serves are safe defaults.
    if (protocols.includes(preset.api)) setProtocol(preset.api)
  ...
  return (
    <div className={styles['editor']}>
      ...
      {/* One-click presets: pick one to fill the form, then adjust and create.
          Disabled once the provider exists (the create is committed). */}
      <div className={styles['presetSection']}>
        <span className={styles['fieldLabel']}>{t('presetTitle')}</span>

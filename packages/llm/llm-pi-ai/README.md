...
Request modalities resolve entry `input` → installed catalog entry → route `defaultInput` (default `[text, image]`), the same order and the same fallback role the capacities above use. An explicit declaration wins over this default, so a catalog model keeps the modalities the catalog records for it, and a deployment that wants the conservative behavior narrows an entry or the route to `[text]`. An entry's empty list means the same as an absent one — it describes a model accepting nothing, so it states no answer and resolution continues past it, which is what keeps a catalog model's own modalities when a `models` entry names it without declaring any. The route's may not be empty, since nothing sits below it to answer instead.

The default is `[text, image]` rather than a conservative `[text]`: image capability is decided by the provider, not by the harness preflight guard, so a text-only model that receives an image fails at the API with its own 400. Over-claiming admits an image the provider then rejects mid-turn — after the message is durable, which leaves the session repeating a request that cannot succeed; a deployment that wants the refusal-before-attach behavior declares `[text]` on an entry or the route.

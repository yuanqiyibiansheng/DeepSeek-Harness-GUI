...
export interface HostApi {
  ...
  /**
   * Read a small text file (the user-global instructions document). The host
   * resolves the target from its own home path — the browser never submits an
   * arbitrary path. Serves `{ content: string | null }`; null when the file
   * does not exist yet.
   */
  readInstructions(
    request: RpcRequest<{}>,
    signal: AbortSignal,
  ): Promise<RpcResponse<{ content: string | null }>>

  /**
   * Write the user-global instructions document (`~/.dsh/AGENTS.md`),

...
import type {
  ...
  ManagedSkillEntry, ModelCatalogFailure, ModelProviderGroup,
...
/** Parse the name/description/toggles out of a SKILL.md body's frontmatter. */
function parseSkillFrontmatter(raw: string): {
  ...
  const fm = raw.slice(3, close).trim()
...
/** Rewrite a SKILL.md body's frontmatter toggles, preserving everything else. */
function rewriteSkillToggles(raw: string, toggle: { modelInvocable?: boolean; userInvocable?: boolean }): string {
  ...
  let next = fm
  if (toggle.modelInvocable !== undefined) {
    const has = /(^|\n)\s*disable-model-invocation\s*:/.test(fm)
    const line = `disable-model-invocation: ${toggle.modelInvocable ? 'false' : 'true'}`
    next = has ? next.replace(/(^|\n)\s*disable-model-invocation\s*:[^\n]*/, `$1${line}`) : `${next}\n${line}`
  }
  if (toggle.userInvocable !== undefined) {
    const has = /(^|\n)\s*user-invocable\s*:/.test(fm)
    const line = `user-invocable: ${toggle.userInvocable ? 'true' : 'false'}`
    next = has ? next.replace(/(^|\n)\s*user-invocable\s*:[^\n]*/, `$1${line}`) : `${next}\n${line}`
  }
  return `---${next}\n---${body}`
}

/** Read and project every user-owned skill, including unloadable entries. */
async function readUserSkills(): Promise<readonly ManagedSkillEntry[]> {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (raw === undefined) {
      skills.push({ name, description: '', modelInvocable: true, userInvocable: true, path: file, loadable: false })
      continue
    }
    ...
    skills.push({
      ...
      path: file,
      loadable: parsed !== undefined,
    })
  }
  return skills
}

/** Rewrite one user skill's frontmatter toggles; undefined when it does not exist. */
async function updateUserSkill(
  name: string,
  toggle: { modelInvocable?: boolean; userInvocable?: boolean },
  ...
  const rewritten = rewriteSkillToggles(raw, toggle)
  if (rewritten !== raw) await writeFile(file, rewritten, 'utf8')
  const parsed = parseSkillFrontmatter(rewritten)
  return {
    name,
    description: parsed?.description ?? '',
    ...parsed?.whenToUse === undefined ? {} : { whenToUse: parsed.whenToUse },
...
/** Delete one user skill (its directory or flat file); false when it does not exist. */
async function removeUserSkill(name: string): Promise<boolean> {
  const root = userSkillRoot()
  const { dir, file } = skillFilePath(root, name)
  try {
  ...
  // The directory may have existed without SKILL.md (not loadable); deleting
  // the dir is what removes the skill either way.
  return true
}
...
/** Curated plugin marketplace offered by the web Plugin Market surface. */
const PLUGIN_MARKET: readonly PluginMarketEntry[] = [
  {
    id: 'dsh-mcp-client',
    title: 'MCP Client',
    author: 'DeepSeek AI',
    description: 'Model Context Protocol client — connect external MCP servers as tools.',
    source: '@deepseek-ai/dsh-mcp-client',
    reference: 'https://github.com/deepseek-ai/deepseek-harness/tree/main/packages/mcp/mcp-client',
    official: true,
  },
  {
    id: 'dsh-web-search',
    title: 'Web Search',
    author: 'DeepSeek AI',
    description: 'Web search capability (DeepSeek search backend) as a tool.',
    source: '@deepseek-ai/dsh-web-search-deepseek',
    reference: 'https://github.com/deepseek-ai/deepseek-harness/tree/main/packages/web/web-search-deepseek',
    official: true,
    bundle: true,
  },
  {
    ...
    author: 'DeepSeek AI',
    description: 'Google Gemini models through the pi-ai provider catalog.',
    source: '@deepseek-ai/dsh-llm-pi-ai',
    reference: 'https://github.com/deepseek-ai/deepseek-harness/tree/main/packages/llm/llm-pi-ai',
    official: true,
    bundle: true,
  },
  {
    ...
    title: 'E2B Sandbox',
    author: 'DeepSeek AI',
    description: 'E2B cloud sandbox for code execution (proof of concept).',
    source: '@deepseek-ai/dsh-e2b',
    reference: 'https://github.com/deepseek-ai/deepseek-harness/tree/main/packages/e2b',
    official: true,
  },
]
...
async function readInstalledPlugins(): Promise<readonly PluginInstalledEntry[]> {
  ...
  const bundles = typeof manifest.dsh === 'object' && manifest.dsh !== null
    && typeof (manifest.dsh as { profile?: unknown }).profile === 'object'
    && (manifest.dsh as { profile: unknown }).profile !== null
    ? (manifest.dsh as { profile: { bundles?: unknown } }).profile.bundles
    : undefined
  const bundleSet = new Set(Array.isArray(bundles) ? bundles.filter((b): b is string => typeof b === 'string') : [])
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    const child = execFile(
      ...
      (error, stdout, stderr) => {
        ...
        if (error) {
          rejectPromise(new Error(combined || error.message))
          return
        }
        resolvePromise()
      },
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {  const sessionExportCompressionLevel = defaults.sessionExportCompressionLevel
  ...
  return {
    ...
    host: {
      ...
      async readInstructions(_request, _signal) {
        try {
          ...
          try {
            ...
            if (hasErrno(error, 'ENOENT')) content = null
            else throw error
          }
          return ok(_request, { content })
        } catch (error: unknown) {
          return err(_request, { code: 'internal', message: `instructions read failed: ${String(error)}`, details: {} })
        }
      },

      async writeInstructions(request, _signal) {
        try {
          const path = instructionsPath()
          await mkdir(dirname(path), { recursive: true })
          await writeFile(path, request.payload.content, 'utf8')
          return ok(request, { written: true })
        } catch (error: unknown) {
          return err(request, { code: 'internal', message: `instructions write failed: ${String(error)}`, details: {} })
        }
      },
    },
    ...
    skills: {
      ...
      // The management methods address only the USER skill root (~/.dsh/skills),
      // the one root the browser UI owns. Project and bundled roots stay
      // untouched: a web user must not be able to edit a checked-in skill.
      async listManaged(request) {
        try {
          const skills = await readUserSkills()
          return ok(request, { skills })
        } catch (error: unknown) {
          return err(request, { code: 'internal', message: `skill management listing failed: ${String(error)}`, details: {} })
        }
      },

      async updateManaged(request) {
        const { name, toggle } = request.payload
        try {
          const updated = await updateUserSkill(name, toggle)
          if (updated === undefined) {
            return err(request, { code: 'skill-not-found', message: `user skill "${name}" not found`, details: { name } })
          }
          return ok(request, updated)
        } catch (error: unknown) {
          return err(request, { code: 'internal', message: `skill update failed: ${String(error)}`, details: {} })
        }
      },

      async removeManaged(request) {
        const { name } = request.payload
        try {
          const removed = await removeUserSkill(name)
          if (!removed) {
            return err(request, { code: 'skill-not-found', message: `user skill "${name}" not found`, details: { name } })
          }
    ...
    pluginMarket: {
      // The marketplace reads the profile manifest for installed state and
      // shells out to `dsh plugin` (a pnpm forwarder) for install/uninstall.
      // Every operation is keyed by the market id, never a free-form spec.
      async snapshot(request) {
        try {
          return ok(request, {
            market: PLUGIN_MARKET,
            installed: await readInstalledPlugins(),
          })
        } catch (error: unknown) {
          return err(request, { code: 'internal', message: `plugin market snapshot failed: ${String(error)}`, details: {} })
        }
      },

      async install(request) {
        const entry = PLUGIN_MARKET.find(market => market.id === request.payload.id)
        if (entry === undefined) {
      ...
      async uninstall(request) {
        ...
        try {
          await runPluginCommand(['remove', entry.source])
          return ok(request, { removed: true })
        } catch (error: unknown) {
          return err(request, { code: 'internal', message: `plugin removal failed: ${String(error)}`, details: {} })
        }
      },
      ...
      async update(_request) {
        try {
          ...
          return ok(_request, { updated: true })
        } catch (error: unknown) {
          return err(_request, { code: 'internal', message: `plugin update failed: ${String(error)}`, details: {} })
        }
      },
    },

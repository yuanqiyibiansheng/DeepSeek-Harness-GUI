 ...
 */
export function clearBrowserState(storage) {
...
export function SettingsCard({ t, onRemoved }) {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (live)
                    setStatus({ version: null, restart: false, channel: 'stable', channels: ['stable', 'beta'] });
            }
            try {
    ...
    const onRemove = useCallback(() => {
        ...
        void (async () => {
            try {
                ...
                if (body.ok === true) {
                    if (purge)
                        clearBrowserState(localStorage);
                    setPhase('removed');
                    onRemoved?.();
                }
                else {
    ...
    const row = (label, hint, action) => h('div', { className: css.setRow }, h('div', { className: css.setLabelBox }, h('div', { className: css.setLabel }, label), h('div', { className: css.setHint }, hint)), action);
    ...
    const body = phase === 'removed'
        ...
        update?.updateAvailable === true && update.latest !== null
            ...
            : update?.channelSwitch != null
                ...
                : t('setSelfUpToDate'), phase === 'updated'
            ...
            : update?.channelSwitch != null
                ...
                : update?.updateAvailable === true ? t('setSelfUpdateHint') : t('setSelfUpToDateHint'), phase === 'updated'
            ? null
            : update?.updateAvailable === true
                ...
                : update?.channelSwitch != null
                    ...
                    : null), row(t('setChannel'), t(CHANNEL_HINT[status?.channel ?? 'stable']), h('div', { className: css.setSeg }, 
        ...
        (status?.channels ?? ['stable', 'beta']).map(id => h('button', {
        ...
        }, t(CHANNEL_LABEL[id]))))), row(t('setSelfRemove'), t('setSelfRemoveHint'), phase === 'confirming' || busy
            ? null
            : h(Button, {
                variant: 'outline',
                size: 'sm',
            ...
            }, t('setSelfRemove'))), phase === 'confirming' || busy
            ? h('div', { className: css.setConfirm }, h('div', { className: css.setHint }, t('setSelfConfirm')), h('label', { className: css.setCheck }, h('input', { type: 'checkbox', checked: purge, onChange: () => { setPurge(!purge); } }), h('span', null, t('setSelfPurge'))), 
            ...
            h('div', { className: css.setHint }, purge ? t('setSelfPurgeOn') : t('setSelfPurgeOff')), h('div', { className: css.setActions }, busy
                ? null
                : h(Button, {
                    variant: 'ghost',
                    size: 'sm',
                    onClick: () => { setPhase('idle'); setPurge(false); },
                }, t('setSelfCancel')), h(Button, {
                variant: 'primary',
                size: 'sm',
                className: css.setDanger,
                disabled: busy,
            ...
            : null, error !== null ? h('div', { className: css.err }, error) : null);
    return h('div', { className: open ? `${css.setCard} ${css.setCardOpen}` : css.setCard }, h('button', {
        type: 'button',
        className: css.setHeader,
        'aria-expanded': open,
        onClick: () => { setOpen(!open); },
    }, h('div', { className: css.setHeadText }, h('div', { className: css.setName }, t('nav'), version !== null ? h('span', { className: css.version }, ` v${version}`) : null, prerelease ? h('span', { className: css.setBetaTag }, t('setChannelBeta')) : null), h('div', { className: css.setDesc }, t('setCardDesc'))), h('span', { className: open ? `${css.setChevron} ${css.setChevronOpen}` : css.setChevron }, h(IconChevronDownOutline14, { size: 14 }))), open ? h('div', { className: css.setBody }, body) : null);
}

...
Compaction-basic resolves each routed target's exact context capacity from the adapter and now computes proactive pressure against the prompt budget left after reserving the model's output capability. `LlmResolvedModelInfo` carries both the provider-owned `context` and the provider-owned `maxTokens` capability; `defaultMaxTokens` remains the adapter-configured per-request cap that LlmRuntime materializes when callers omit one. The pi-ai adapter exposes catalog `maxTokens` as model capability and keeps deployment `defaultMaxTokens` separate. Compact-basic uses that capability only when it can make a positive prompt budget, and its tests cover both the exact-capacity path and the prompt-budget scaling path.
